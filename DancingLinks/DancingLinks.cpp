...
void DancingLinks::Create(int R,int C1,int C2,char *mat)
{
  ...
  for(int i=0,k=0;i<R;i++)
  {
    for(int j=0;j<C;j++,k++)
    {
      ...
      if(k%C==0) cells_[k].set_left_ptr(&cells_[k+C-1]);
      else       cells_[k].set_left_ptr(&cells_[k-1]  );

      if(k%C==C-1) cells_[k].set_right_ptr(&cells_[k-C+1]);
      else         cells_[k].set_right_ptr(&cells_[k+1]  );
      ...
      else cells_[k].set_up_ptr(&cells_[k-C]);
      ...
      else cells_[k].set_down_ptr(&cells_[k+C]);
    }
  }
    ...
    std::vector<int>& cursolution,bool allneeded)
{
      ...
      ptr!=chosencolumn->dummycell();ptr=ptr->down_ptr())
  {
    ...
    for(DancingLinksCell *ptr2=ptr->right_ptr();
        ptr2!=ptr;ptr2=ptr2->right_ptr())
    {
      ptr2->column_ptr()->RemoveColumn();
    }
    
    //Recurse to solve the modified board
    if(solve(allsolutions,cursolution,allneeded))
      flag=true;
    ...
    //If only one solution needs to be found and we have found one, don't bother
