...
int main()
{
  ...
  int R=horiz_dom_count+vert_dom_count,C1=pos_count+dom_type_count,C2=0,C=C1+C2;
  ...
  for(int i=0;i<H-1;i++)
  {
    for(int j=0;j<W;j++)
    {
      ...
      int c3=H*W+board[i][j]*L+board[i+1][j],c4=H*W+board[i+1][j]*L+board[i][j];
